  ...
  run: async (client, message, query) => {
    ...
    try {
      if (!config.admin.includes(message.author.id))
        return message.channel.send("Only bot owner can use this");
      ...
      m.createReactionCollector(filter, { time: 600000, max: 1 }).on(
        "collect",
        async col => {
      ...
      const filter = (rect, usr) =>
        choice.includes(rect.emoji.name) && usr.id === message.author.id;
      m.createReactionCollector(filter, { time: 60000, max: 1 }).on(
        "collect",
        async col => {
          if (col.emoji.name === "🚫") return m.delete();
        }
